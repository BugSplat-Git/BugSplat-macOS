...
sub getSymbolPathFor_dsymUuid{
    ...
    foreach my $exec_name (@exec_names) {
        ...
        foreach my $app_bundle (@app_bundles) {
            if( -f "$app_bundle/Contents/MacOS/$exec_name") {
                push(@exec_paths, "$app_bundle/Contents/MacOS/$exec_name");
            }
        }
        ...
        foreach my $archive_path (@archive_paths) {
            my $cmd = "find \"$archive_path/Products/Applications\" -name \"$exec_name.app\"";
            print STDERR "Running $cmd\n" if $opt{v};
            
            foreach my $app_bundle (split(/\n/, `$cmd`)) {
            print STDERR "\$app_bundle: $app_bundle" if $opt{v};
                if( -f "$app_bundle/Contents/MacOS/$exec_name") {
                    push(@exec_paths, "$app_bundle/Contents/MacOS/$exec_name");
                }
            }
    ...
    print STDERR "\@exec_paths: @exec_paths" if $opt{v};
...
sub getSymbolPathFor {
    ...
    print STDERR "\$path: $path"  if defined($path) and $opt{v};
    ...
    if ( !defined($out_path) || !matchesUUID($out_path, $uuid, $arch)) {
        ...
        for my $func (
            \&getSymbolPathFor_searchpaths,
                ...
                print STDERR "\@out_path_arr: @out_path_arr" if $opt{v};
...
sub parse_images {
    ...
    my $architectures = "armv[4-8][tfsk]?|arm64|x86_64";
...
sub fetch_symbolled_binaries {
    ...
    for my $b (keys %$images) {
        ...
        print STDERR "\r\$lib: $lib" if $opt{v};
